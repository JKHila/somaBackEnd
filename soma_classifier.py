...
#train_df = pd.read_table('df_test_analysis.txt', sep='$', header=None, names=['cate1','cate2','cate3','name'])
train_df
...
for each in train_df.iterrows():
    ...
    tmp_name = each[1]['name']
    #for key in removeList:
    #    if tmp_name.find(key):
    #        tmp_name =  tmp_name.replace(key,"")
    analysis = twitter.morphs(tmp_name)
    for key in analysis:
        if len(key)==1:
                analysis.remove(key)     
        """if result:
    ...
    #print map(str, analysis)
    str2=""
    for each in ngramList:
        str2 += ''.join(each)
        str2 +=' '
    str1 = ' '.join(analysis)
    str3 = tmp_name+' '+str1+' '+str2
    #for key in removeList:
    #    if str1.find(str(key)):
    #        str1.replace(str(key),"")
    d_list.append(str3)
    #print(hannanum.nouns(each[1]['name']))
    cate_list.append(cate)
#    total_list.append(cate+'$'+each[1]['name']+'\n')
